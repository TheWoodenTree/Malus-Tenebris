...
func _on_safe_room_area_body_entered(body):
	if body == Global.player:
		...
		#tween.parallel().tween_property(music_player, "volume_db", -15, 3.0).from(-50).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_SINE)
		#music_player.play()
...
func _on_safe_room_area_body_exited(body):
	if body == Global.player:
		...
		#tween.parallel().tween_property(music_player, "volume_db", -35, 3.0).from(-15).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
