...
#
#@onready var highlight_material = $door.material_overlay
#@onready var door_open_player = $door_open_player
#@onready var door_open_finish_player = $door_open_finish_player
#@onready var door_unlock_player = $door_unlock_player
#@onready var door_close_player = $door_close_player
#@onready var door_attempt_player = $door_attempt_player
#@onready var interact_area = $interact_area
#@onready var key_anim_player = $key_anim_player
#@onready var key = $key # Parent necessary because of a bug relating to scale when setting global_rotation
...
#func _physics_process(_delta):
	#if not sleeping and not closed:
		#if abs(angular_velocity.y) > 0.1:
			#var effect_scale: float = clamp(abs(angular_velocity.y) / PI, 0, 1.0)
			#var large_ang_vel_change: bool = abs(angular_velocity.y - angular_velocity_last_frame.y) > 0.35
			#var ang_vel_dir_changed: bool = sign(angular_velocity.y) != sign(angular_velocity_last_frame.y)
			...
			#if not player_dragging and abs(rotation.y - deg_to_rad(angle_offset)) <= deg_to_rad(abs(closed_max_drag_angle)):
				#set_closed(true)
			#
			...
			#
			#angular_velocity_last_frame = angular_velocity
		#
		## Increase damp for angular velocity when the door is close to its hinge limit
		#if sign(open_to_angle + angle_offset) == sign(angular_velocity.y):
			#var min_damp_angle: float = abs(open_to_angle + angle_offset) - 10.0
			#var max_damp_angle: float = abs(open_to_angle + angle_offset)
			#var normalized: float = (abs(rad_to_deg(rotation.y)) - min_damp_angle) / (max_damp_angle - min_damp_angle)
			#var damping_scale: float = clamp(normalized, 0.0, 1.0) * 10.0
			#angular_damp = clamp(5.0 * damping_scale, 5.0, 50.0)
		#elif not closed:
			#angular_damp = 5.0
		#
		...
		#set_closed(false)
		#
	#if abs(angular_velocity.y) < 0.05:
...
#
					...
					#attempt_open_angle = -0.0138*PI
#
				#door_tween.tween_property(self, "rotation:y", attempt_open_angle, 0.1)
				#door_tween.tween_property(self, "rotation:y", 0, 0.1)
...
#func add_torque_to_door(offset: Vector2):
		...
		#var torque: Vector3 = Vector3.UP * cam_rot_offset.y * 150.0 * torque_sign
		#apply_torque(torque)
...
#func get_player_z_dist():
	#return get_parent().to_local(Global.player.global_position).rotated(Vector3.UP, rotation.y).z
#
#
