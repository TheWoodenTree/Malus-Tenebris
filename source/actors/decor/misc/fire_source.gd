...
func _on_fire_burning_sound_area_entered():
	if lit and fire.has_node("BurningPlayer"):
		var start_time: float = Global.torch.fire.burning_player.get_playback_position() + 15.0
		start_time = wrapf(start_time, 0.0, Global.torch.fire.burning_player.stream.get_length())
